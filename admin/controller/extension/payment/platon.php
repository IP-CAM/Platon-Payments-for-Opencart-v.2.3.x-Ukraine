  ...
  private $gw_url = 'https://secure.platononline.com/payment/auth';
	...
	public function index() {
		...
		if (isset($this->error['gw_url'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/payment/platon', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

		$data['action'] = $this->url->link('extension/payment/platon', 'token=' . $this->session->data['token'], 'SSL');
      ...
      $cfg_gw_url = $this->config->get('platon_gateway_url');
		...
		$this->response->setOutput($this->load->view('extension/payment/platon.tpl', $data));
	}
